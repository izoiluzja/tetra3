...
def test_insert_at_index():
    ...
    hash_index = 2
    index = utils._insert_at_index(pattern, hash_index, table)
...
def test_get_table_index_from_hash():
    ...
    pattern = np.array([1, 2, 3, 4, 5], dtype=np.uint64)
    hash_index = 2
    utils._insert_at_index(pattern, hash_index, table)
    indices = utils._get_table_index_from_hash(hash_index, table)
...
def test_key_to_index():
    ...
    bin_factor = 50
    max_index = 1000
    hash_index = utils._key_to_index(key, bin_factor, max_index)
...
def test_compute_vectors():
    ...
    size = (400, 400)
    fov = np.deg2rad(10)
    vectors = utils._compute_vectors(centroids, size, fov)
...
def test_compute_centroids():
    ...
    size = (400, 400)
    fov = np.deg2rad(10)
    centroids, indices = utils._compute_centroids(vectors, size, fov, trim=True)
...
def test_undistort_centroids():
    ...
    size = (400, 400)
    k = -0.1
    undistorted = utils._undistort_centroids(centroids, size, k)
...
def test_distort_centroids():
    ...
    size = (400, 400)
    k = 0.1
    distorted = utils._distort_centroids(centroids, size, k)
...
def test_find_rotation_matrix():
    ...
    rotation_matrix = utils._find_rotation_matrix(image_vectors, catalog_vectors)
...
def test_find_centroid_matches():
    ...
    r = 10
    matches = utils._find_centroid_matches(image_centroids, catalog_centroids, r)
